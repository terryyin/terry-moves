...
import { CurrentSubtitle1 } from './models/CurrentSubtitle';
import { StageTransform, sinceSubtitle, useCurrentStage, useCurrentSubtitle1 } from './hooks/useCurrentSubtitle';
...
export const SceneCustomer: React.FC = autonomousComponent(({frame, fps}) => {
	const currentSubtitle1: CurrentSubtitle1 = useCurrentSubtitle1(subtitles, frame, fps);
	const viewPosition = useCurrentStage(StageTransforms, currentSubtitle1.currentSubtitle);

  return (
    <Sequence  durationInFrames={73 * 30}>
      <Stage viewPosition={ viewPosition}>
				<Company currentSubtitle={currentSubtitle1.currentSubtitle} style={{position: 'absolute', left: '5%', top:'10%', width: '45%', height: '100%'}}/>
				<CustomerGroup currentSubtitle={currentSubtitle1.currentSubtitle} style={{position: 'absolute', left: '70%', top:'15%', width: '25%', height: '100%'}} />
				<div style={{position: 'absolute', left: '45%', top: '35%', width: '25%', height: '25%'}}>
					{sinceSubtitle(currentSubtitle1.currentSubtitle, "intro1") && <ValueArrow />}
				</div>
				<div style={{position: 'absolute', left: '45%', top: '50%', width: '37%', height: '37%'}}>
					{sinceSubtitle(currentSubtitle1.currentSubtitle, "customerResources1") && <MoneyArrow />}
				</div>
			</Stage>
			<Subtitles currentSubtitle={currentSubtitle1.currentSubtitle}/>
    </Sequence>
  );
});